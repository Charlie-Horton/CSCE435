...
double standard_deviation;         //Standard deviation of list 
...
void *find_minimum (void *s) {
    ...
    // Thread computes total of list[my_start ... my_end]
    long long int my_total = 0;
    for (j = my_start+1; j <= my_end; j++) my_total += (long long int)list[j];

    //printf("my_total: %llu\n", my_total);
    double my_mean = (double)my_total / (double)(my_end - my_start);
    //printf("my_mean: %f\n", my_mean);

    // Thread computes and sums square of list[my_start ... my_end] distance to my_mean
    long double my_distance = 0;
    for(int j = my_start+1; j <= my_end; j++){
        my_distance += pow(list[j] - my_mean, 2);
    }

    // Thread updates global mean and standard deviation
    pthread_mutex_lock(&lock_minimum);
    count++;
    mean = mean * (count-1)/count + my_mean/count;
    standard_deviation += my_distance;
