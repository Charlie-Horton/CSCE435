...
#pragma omp flush(found)
    ...
    // My Approach
*/
    printf("Starting My Approach\n");
    start_time = omp_get_wtime();
    volatile int found = 0;

    #pragma omp parallel sections default(none) shared(MyLawn, found)
    {
        #pragma omp section    //Top Left
        {
            int x = 0;
            int y = 0;
            double maxAnts = MyLawn.number_of_ants_in_cell(x, y);
            while(found == 0){
                if(maxAnts < MyLawn.number_of_ants_in_cell(x+1, y)){
                    x++;
                    maxAnts = MyLawn.number_of_ants_in_cell(x, y);
                }
                else if(maxAnts < MyLawn.number_of_ants_in_cell(x, y+1)){
                    y++;
                    maxAnts = MyLawn.number_of_ants_in_cell(x, y);
                }
                else{
                    printf("Top Left looking at %d %d\n", x, y);
                    if(MyLawn.guess_anthill_location(x, y)){
                        found = 1;
                        #pragma omp flush(found)
                        printf("Found by Top Left\n");
                    }
                    else{
                        x++;
                        y++;
        ...
        #pragma omp section     //Top Right
        {
            ...
            while(found == 0){
                if(maxAnts < MyLawn.number_of_ants_in_cell(x-1, y)){
                    ...
                    printf("Top Right looking at %d %d\n", x, y);
                    if(MyLawn.guess_anthill_location(x, y)){
                        found = 1;
                        #pragma omp flush(found)
                        printf("Found by Top Right\n");
                    }
                    else{
                        x--;
                        y++;
        ...
        #pragma omp section     //Bottom Left
        {
            int x = 0;
            int y = MyLawn.m-1;
            double maxAnts = MyLawn.number_of_ants_in_cell(x, y);
            while(found == 0){
                if(maxAnts < MyLawn.number_of_ants_in_cell(x+1, y)){
                    ...
                    printf("Bottom Left looking at %d %d\n", x, y);
                    if(MyLawn.guess_anthill_location(x, y)){
                        found = 1;
                        #pragma omp flush(found)
                        printf("Found by Bottom Left\n");
                    }
                    else{
                        x++;
                        y--;
        ...
        #pragma omp section     //Bottom Right
        {
            int x = MyLawn.m - 1;
            int y = MyLawn.m - 1;
            double maxAnts = MyLawn.number_of_ants_in_cell(x, y);
            while(found == 0){
                if(maxAnts < MyLawn.number_of_ants_in_cell(x-1, y)){
                    ...
                    printf("Bottom Right looking at %d %d\n", x, y);
                    if(MyLawn.guess_anthill_location(x, y)){
                        found = 1;
                        #pragma omp flush(found)
                        printf("Found by Bottom Right\n");
                    }
                    else{
                        x--;
                        y--;
